...
class SemanticModelGenerator {
  ...
  getModels(mapping, k) {
    ...
    while (!iteratorHeap.empty()) {
      ...
      let crossProduct = SemanticModelGenerator._crossProduct(iteratorVisit, v.list, iteratorVisit);
      ...
      crossProduct.forEach(function (iteratorVisits) {
        let resultTree = self.getResultTree(iteratorVisits);
