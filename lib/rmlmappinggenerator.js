      ...
      edges.forEach(function(edge){
        ...
        if (targetNode.type === type.DATAREFERENCE) {
          ...
          edge.joinConditions.forEach(condition => {
            let blankNode = '_:b' + this.blankNodeCounter;
            this.blankNodeCounter ++;

            self.triples.push({subject: blankNode, predicate: namespaces.rr + 'child', object: '"' + condition.child + '"'});
            self.triples.push({subject: blankNode, predicate: namespaces.rr + 'parent', object: '"' + condition.child + '"'});
            self.triples.push({subject: om, predicate: namespaces.rr + 'joinCondition', object: om});
          });
        }
      });
    });
