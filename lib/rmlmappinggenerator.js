...
class RMLMappingGenerator {
  ...
  _generateSubjectMaps(nodes) {
    ...
    nodes.forEach(function(node){
      ...
      if (node.isBlankNode) {
        self.triples.push({subject: sm, predicate: namespaces.rr + 'termType', object: namespaces.rr + 'BlankNode'});
  ...
  /**
   * This method generates the logical sources for the nodes.
   * @param nodes
   * @private
   */
  _generateLogicalSources(nodes) {
    ...
    nodes.forEach(function(node){
      ...
      //we only generate a logical source when we know about the source
  ...
  /**
   * This method returns the source description of a node.
   * @param node
   * @returns {*}
   * @private
   */
  _getSourceDescription(node) {
    //if the node has a source description return it
    if (node.sourceDescription !== undefined) {
      return node.sourceDescription;
    } else {
      //if not, we look into the connected nodes and see if one of those has a source description
      let edges = this.semanticmodel.getEdgesWithNodeAsSource(node.id);

      for (let i = 0; i < edges.length; i ++) {
        let edge = edges[i];
        let targetNode = this.semanticmodel.get(edge.target);
