...
class SemanticModel {
  ...
  /*
    create new node using @node
    node.id is overwritten
  ...
  /*
    return: all edges where the source node is of the type @sourecType and target node is of the type @targetType
   */
  ...
  /*
    return: edge that has as source node @node and that has label @label
   */
  ...
  /*
    return: node with the largest set of .tags from the array @nodes
   */
  ...
  addSemanticTypes(semanticTypes) {
    for (let i = 0; i < semanticTypes.length; i++) {
      ...
      //look for the class node of the semantic type
      let vMatch = this.getNodes(type.CLASS, lV);

      if (vMatch.length === 0) {
        //no class node found, so we add one
        this.createNode({
          type: type.CLASS,
          label: lV
        });
      ...
      for (let j = 0; j < vMatch.length; j++) {
        ...
        if (!outgoingEdge) {
          //such edge not found, so we add a data node and edge
